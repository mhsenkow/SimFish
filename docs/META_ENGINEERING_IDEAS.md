...
- [x] **1. Active inference as the action-selection core.** Today `mind_world_model.gd`'s `expected_free_energy_explore()` computes info-gain but is read-only. Make **expected free energy** the bid currency in `global_workspace.gd`: fish act to minimize surprise *and* seek information. This single change turns curiosity, foraging, and caution into one principled objective instead of three hand-tuned drives. *XL · XL* — **LANDED 2026-06-29 (Phases 0-3):** full architecture + phased, flag-gated, eval-gated migration in [docs/ACTIVE_INFERENCE_CORE.md](ACTIVE_INFERENCE_CORE.md). `mind_active_inference.gd` = the unified objective (pragmatic + epistemic + precision → `efe_salience`, a drop-in for hand-tuned bid salience); `smoke_active_inference_core.gd` proves the unification (one function: food-dominant when hungry, explore-dominant when sated+curious) and the legacy-reproduction safety property. Phase 1 (EFE→bid salience), Phase 2 (EFE→DDM drift/threshold), Phase 3 (flag default on, legacy kept only as rollback) all landed; `smoke_mind_eval.gd` **14/14 + honesty PASS** (incl. new S3 pragmatic+epistemic alignment). Rollback = flip the flag off.
...
- [x] **3. Upgrade the world model from 6-D linear to a tiny recurrent predictor.** Replace the linear map in `mind_world_model.gd:45–63` with a 16–32 state GRU-lite (hand-rolled, no deps) that predicts next interoceptive state. Persists per-fish; learns multi-step dynamics, not just decay constants. *L · L* — **Shipped 2026-06-28 (Tier-1/1B):** `mind_world_model.gd` Minimal Gated Unit (1 forget gate, 8 hidden), seeded per-fish from id (deterministic), recurrent prediction error folds a small centered nudge into `variance` (sharpens active inference #1); save migration + `ensure_model` backfill. Smoke `smoke_world_model_gru.gd`.
...
- [x] **8. Sleep that consolidates episodic → semantic.** `episodic_memory.gd` does FIFO pruning + TD heatmap replay. Add real systems-consolidation: cluster episodes into reusable schemas during the night rest state so a fish wakes with *generalized* rules ("the top-left is dangerous at dawn"), not just raw episodes. *L · L* — **Shipped 2026-06-29 (Tier-1):** `episodic_memory.consolidate_sleep` now distils positioned same-kind episodes into structured SPATIAL schemas `{kind, center, valence, strength}` (`_semantic_schemas` on Fish); `schema_valence_at` scores a location and `collect_schema_bid` raises a caution bid when the fish re-enters a learned-dangerous region — acting on a generalized rule, not a single memory. Smoke `smoke_sleep_consolidation.gd`.
...
- [x] **12. A bid-generator registry (plugin attention).** `global_workspace.gd` hardcodes its bid sources. Make bid generators registerable so a new drive (territoriality, play, grief) is a 30-line plugin, not a kernel edit. Mods and experiments slot in without touching the core. *M · L* — **Shipped 2026-06-29:** `GlobalWorkspace.register_bid_generator(Callable(f,sim)->Array)` + `_append_registered` run plugin generators on top of the built-in bids (additive, same precision/coalition pipeline, malformed generators skipped). Smoke `smoke_bid_registry.gd`. Built-in bids not yet migrated to generators — additive hook first.
- [x] **13. Module dependency DAG + topological tick order.** The felt-self spine order is enforced by hand in `felt_self_layer.gd`. Declare module deps as data and topo-sort the tick — adding a module can't silently break ordering, and the order becomes inspectable/testable. *M · M* — **Shipped 2026-06-29:** `mind_module_graph.gd` declares the spine deps as data (`DEPS`) + deterministic Kahn `topo_sort`/`topo_order` with cycle + dangling-prereq rejection; `smoke_module_graph.gd` validates the real spine (protoself first, binding after its inputs, encode last, cycles rejected, new module slots in). Spec/guard now; live data-driven dispatch is a follow-up.
...
- [x] **17. Mind schema versioning + migration chain.** `MindState.SCHEMA_VERSION=3` exists but there's no migration. Add a `MindMigration` ladder so a v3 save loads into a v5 kernel with new felt-self fields defaulted — no silent `.get(k, default)` degradation. *M · M* — **Shipped 2026-06-29:** `mind_migration.gd` ladder (`migrate(d, current)` walks v_n→v_n+1 with explicit per-step field defaults, never downgrades); `MindState.SCHEMA_VERSION` 3→4 and `from_dict` runs the ladder first. Smoke `smoke_mind_migration.gd`.
...
- [x] **20. Cognition LOD tiers as a first-class concept.** Define explicit tiers (T0 reflex-only, T1 GWT, T2 +world-model, T3 +LLM voice) and assign by salience/visibility/budget. The off-frustum phase hack in `sim_driver.gd:3222` becomes a principled scheduler input instead of a special case. *M · L* — **Shipped 2026-06-29:** `mind_lod.gd` — `tier_for(f, visible, budget_pressure)` → T0 reflex / T1 GWT / T2 +world-model / T3 +voice from status (guardian/named/familiar) + visibility, demoted under budget pressure; `runs_workspace/world_model/voice` gates. Smoke `smoke_mind_lod.gd`. Pure tier model + assignment (tested); wiring sim_driver/mind_cycle to skip phases by tier (and #19's richer-model promotion) is the follow-up integration.
