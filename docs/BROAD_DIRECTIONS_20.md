...
| 2026-09-12 | plants | ✅ | **"Plants reach the surface" was quietly true only on small tanks.** An established plant starts at 72% of mature height; on a small tank that lands past the waterline, on a tall one it lands short — 10 voxels short at height 20 — and it then has to grow there against a per-tick budget shared by every plant in the tank. Surface-reaching species now start **at** the waterline with a jittered share of the surplus already laid over (which also keeps the stand ragged). `MAX_PLANT_VOXELS` 52 → 72, since height 20 needs 47 voxels before any surplus | Measured on an 18-unit tank: 34 plants, 10 at the surface, tops 17.44–17.76 vs water 16.74, all in CANOPY |
| 2026-09-12 | hardscape | ✅ | **There was exactly one piece of wood.** A single bezier trunk arcing low across the floor plus four hard-coded twigs — every tank the same silhouette, hardscape "styles" only scaling how many voxels it was made of, and because the trunk hugs the substrate the whole middle and upper column stayed empty. Added `DriftwoodForm`: `log` (the original, reproduced exactly), `branch` (climbs and forks up through the water column), `spider` (low hub, limbs radiating outward), `stump` (squat root mass). Styles now pick a **shape**, not just a count. Wood uses its own RNG so it cannot shift the world placement stream | `smoke_driftwood_form.gd` 347 checks; branch verified by render |
| 2026-09-12 | hardscape | ⚠️→✅ | The new `log` form initially used *fractions of the water column* where the original used **absolute** offsets (`+1.25`/`+1.55` above substrate). On a sphere vessel those land somewhere quite different, which moved hardscape occupancy and pushed polyp_lab's carpet planting out of bounds — caught by `smoke_scenario_layouts`. Also removed a gratuitous random mirror I had added to the same form. Both were changes to something I had no reason to change | Layout smoke green |
| 2026-09-12 | hardscape | ✅ | **The wood forms had no way in.** `hardscape_style` is not a `TankConfig` property and no panel referenced it, so a form was reachable only by picking a template that happened to use it — and `spider` shipped as **dead code**, selected by nothing. Added `wood_form` to TankConfig (default `"auto"`, so existing tanks keep the shape they were designed with) with precedence **Settings override → preset's own `wood_form` → style default**, plus a Settings → Tank dropdown (auto / log / branch / spiderwood / stump / none). Assigned forms to templates: shrimp_sanctuary → spider, valli_jungle and night_lamp → branch | Gated: every form must be reachable without editing source. Spider rendered for the first time |
| 2026-09-12 | bugs | ✅ | **"Left operand of 'is' is a previously freed instance"** — the guard was in the right place but **one term too late**. `if partner != null and f.get("has_mate") and partner is Node3D and is_instance_valid(partner)`: `and` short-circuits left to right, so `is` ran on the dead reference and threw before the validity test beside it could help. A bonded mate dying is routine — fish live ~4 minutes and bonds outlive them — so this fired constantly in the per-tick mind system | `global_workspace.gd:392` |
| 2026-09-12 | bugs | ✅ | **It was a class, not an instance: 30 sites had the same ordering.** Worst were three guards written specifically to DETECT a freed node — `if state is Node and not is_instance_valid(state): return null` in `mind_legible`, `mind_panel` and `sim_gate` — which throw on precisely the input they exist to catch, so they could never once do their job. Those collapse to the validity test alone (the params are already typed `Object`, so it is both correct and stricter). The other 26 were behaviour-preserving reorders | 456 scripts compile |
| 2026-09-12 | testing | ✅ | Added `smoke_freed_instance_guard.gd` — a **lint**, not a unit test, because the defect is a source *ordering* invisible to any behavioural assertion: the crash only happens when a real object is freed at the wrong moment. Scans every script, joins line continuations (the reported bug spanned two lines), and fails on any `is` appearing before its own `is_instance_valid` on the same statement | Verified by reinstating the original line — fails, naming it |
