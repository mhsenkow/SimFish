...
func _initialize() -> void:
	...
	_assert(failed, is_equal_approx(PerfGovernor.target_frame_ms, PerfGovernor.TARGET_FRAME_MS),
	...
	_assert(failed, on_target_uncapped < 0.2,
		"steady 60 fps uncapped -> slack (got %.2f)" % on_target_uncapped)
	_assert(failed, not PerfGovernor.governor_step_down() and not PerfGovernor.governor_step_up_block(),
		"on-target uncapped device trips no governor gate")
	for i in 90:
		PerfGovernor.record_frame(1.0 / 30.0)
	_assert(failed, PerfGovernor.budget_pressure > 0.6,
	...
	PerfGovernor.reset_for_test()
	PerfGovernor.refresh_target()
	_assert(failed, PerfGovernor.target_frame_ms > 30.0,
	...
	_assert(failed, absf(on_target_capped - on_target_uncapped) < 0.02,
		"30 fps under a 30 fps cap reads the same as 60 under 60 (%.3f vs %.3f)"
			% [on_target_capped, on_target_uncapped])
	_assert(failed, not PerfGovernor.governor_step_down(),
		"capped-but-healthy device does not trip the step-down gate")
	_assert(failed, not PerfGovernor.governor_step_up_block(),
	...
	Engine.max_fps = 5
	PerfGovernor.refresh_target()
	_assert(failed, PerfGovernor.target_frame_ms <= 50.0,
	...
	_assert(failed, src.contains("target_fps = minf(target_fps, float(Engine.max_fps) * 0.97)"),
		"adaptive quality target is clamped to the fps cap")
	_assert(failed, src.contains("func _seed_render_budget_for_tier"),
		"first launch seeds a render budget from the device tier")
	_assert(failed, src.contains('"low": Vector2i(384, 216)'),
