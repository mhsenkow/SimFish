...
func _build_ui() -> void:
	...
	body.add_child(_projection_option)

	body.add_child(PanelTheme.make_rule())
	body.add_child(PanelTheme.make_section("Saved views"))
	for i in NUM_VIEW_SLOTS:
		...
		slot_label.add_theme_color_override("font_color", PanelTheme.LABEL_FG)
	...
	speed_val.text = "0.08"
	_auto_orbit_speed = PanelTheme.add_slider_row(body, "Orbit speed", 0.02, 0.4, 0.01, speed_val)
	_auto_orbit_speed.value = 0.08
	_auto_orbit_speed.value_changed.connect(func(v: float):
		speed_val.text = "%.2f" % v
	...
	body.add_child(PanelTheme.make_rule())
	body.add_child(PanelTheme.make_section("Lens"))
	...
	_fov_label.custom_minimum_size = Vector2(56, 0)
	_fov_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
...
func _on_pond_pressed() -> void:
	if main_ref != null and main_ref.has_method("apply_pond_mode"):
		var on: bool = true
		if main_ref.has_method("is_pond_mode"):
			on = not bool(main_ref.is_pond_mode())
		main_ref.apply_pond_mode(on)
