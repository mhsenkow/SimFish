...
func _initialize() -> void:
	...
	TestSupport.check(failed, clear.x > clear.y and clear.y > clear.z,
	...
	TestSupport.check(failed, is_equal_approx(abs4.w, 0.62), "strength rides in absorb.a")
	TestSupport.check(failed, is_equal_approx(body4.w, 6.5), "water surface Y rides in body.a")
	TestSupport.check(failed, abs4.x > abs4.y and abs4.y > abs4.z,
	...
	TestSupport.check(failed, is_equal_approx((offu[0] as Vector4).w, 0.0),
	...
	TestSupport.check(failed, is_equal_approx(_Vox.water_transmittance(kr, 0.0, 20.0, 0.62), 1.0),
		"nothing above the waterline is touched")
	TestSupport.check(failed, is_equal_approx(_Vox.water_transmittance(kr, 5.0, 20.0, 0.0), 1.0),
	...
	TestSupport.check(failed, inc.contains("if (depth <= 0.0)"),
		"emergent growth and floaters skip the water column")
	TestSupport.check(failed, inc.contains("if (strength < 0.001)"),
		"the effect short-circuits when disabled")
	TestSupport.check(failed, inc.contains("view_dist * 0.35"),
	...
	TestSupport.check(failed, wsrc.contains("cfg = get_node_or_null(\"/root/TankConfig\")"),
