...
# Boundary + collision-avoidance steering now lives in FishLocomotion (#1 / 0C).
# These stay as thin delegates so Tier-0 / Tier-0.1 call sites are unchanged.
func _wall_avoid(_b: AABB) -> Vector3:
	return FishLocomotion.wall_avoid(self, _b)


func _local_clearance_push(neighbors: Array, plants: Array) -> Vector3:
	return FishLocomotion.local_clearance_push(self, neighbors, plants)


func _hardscape_clearance_push() -> Vector3:
	return FishLocomotion.hardscape_clearance_push(self)
