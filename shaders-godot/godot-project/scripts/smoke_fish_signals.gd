...
func _initialize() -> void:
	...
	TestSupport.check(failed, FishSignals.active_signal(a) == FishSignals.ALARM, "spooked fish emits ALARM")

	var mfish: Fish = _mk("mate-src")
	mfish.partner = _mk("partner")
	FishSignals.tick(mfish, 0.1)
	TestSupport.check(failed, FishSignals.active_signal(mfish) == FishSignals.MATE, "paired fish emits MATE")
	...
	TestSupport.check(failed, str(FishSignals._state(b).get("heard", "")) == FishSignals.ALARM,
	...
	TestSupport.check(failed, str(FishSignals._state(far).get("heard", "")) == "",
	...
	TestSupport.check(failed, FishSignals.interpret(b, FishSignals.ALARM) > 1.0,
	...
	TestSupport.check(failed, sal1 > sal0, "learned trust raises the heard-alarm bid salience")

	# --- Distrust pulls a signal's weight down.
	for _j in 60:
		FishSignals.reinforce(b, FishSignals.FOOD, false)
	TestSupport.check(failed, FishSignals.interpret(b, FishSignals.FOOD) < 0.5,
			"discounted signals lose trust")

	# --- Integration: a heard alarm shows up in collect_bids (intersubjectivity).
	FishSignals.scan(b, [a])
	TestSupport.check(failed, _has_bid(GlobalWorkspace.collect_bids(b, null), "threat"),
			"heard alarm enters the Global Workspace via collect_bids")

	quit(TestSupport.report("smoke_fish_signals", failed))
