...
"""
	...
	for i in mats.size():
		...
		TestSupport.check(failed, is_equal_approx(float(mat.get_shader_parameter("daylight")), 0.73),
			"material %d missed daylight update" % i)
		TestSupport.check(failed, is_equal_approx(float(mat.get_shader_parameter("flow_strength")), 0.42),
			"material %d missed flow update" % i)
	TestSupport.check(failed, VoxelMat._live_foliage_mm_mats().size() >= 128,
		"registry retains all living materials")
	mats.resize(2)
	# Weak entries must not keep discarded materials alive.
	VoxelMat._live_foliage_mm_mats()
	TestSupport.check(failed, VoxelMat._foliage_mm_mats.size() < 128,
	...
	TestSupport.check(failed, bounds.position.x <= -5.0 and bounds.end.x >= 6.5,
		"dynamic bounds enclose transformed voxels and margin")
	TestSupport.check(failed, bounds.size.x < 20.0 and bounds.size.y < 20.0,
		"dynamic bounds replace oversized legacy AABB")
	batch.clear()
	TestSupport.check(failed, batch.mmi.custom_aabb.size.x <= 2.01,
	...
	TestSupport.check(failed, plant_range > Plant.PLANT_LOD_BASE_RANGE,
	...
	while not stack.is_empty():
		...
		for child in node.get_children():
			if child is GeometryInstance3D:
				...
				ranged_geometry += 1
				TestSupport.check(failed, is_equal_approx(geometry.visibility_range_end, plant_range),
					"rooted geometry receives plant visibility range")
			if child.get_child_count() > 0:
				stack.append(child)
	TestSupport.check(failed, ranged_geometry > 0, "plant smoke built ranged geometry")
	...
	stem_plant.init(6, {"max_height": 12, "leaf_form": "column",
		"asymmetry_seed": 4242})
	TestSupport.check(failed, stem_plant._stem_batch != null
			and stem_plant._stem_batch.mmi != null,
		"multi-voxel rooted plant creates a stem batch")
	TestSupport.check(failed, stem_plant.voxels.size() == 6,
	...
	TestSupport.check(failed, all_batched, "every structural stem points at the shared batch")
	...
	TestSupport.check(failed, stem_bounds.size.y > Plant.VOXEL_SIZE * 4.0
	...
	TestSupport.check(failed, eaten == 1 and not removed_handle.alive,
		"grazing hides the removed stable stem handle")
	TestSupport.check(failed, stem_plant.biomass() == stem_plant.current_height
	...
	TestSupport.check(failed, int(far_plant.get("tick_calls")) == 1,
	...
	TestSupport.check(failed, sim._plant_tick_divisor(far_plant, camera) == 1,
		"stressed far plant returns to full tick rate")
	host.queue_free()
	quit(TestSupport.report("smoke_plant_render_registry", failed))
