...
func _initialize() -> void:
	...
	TestSupport.check(failed, not old.alive and replacement.alive,
		"migration swaps stable lifecycle handle")
	TestSupport.check(failed, replacement.batch == plant._senescence_batch,
		"late leaf uses secondary shared batch")
	TestSupport.check(failed, replacement.custom_data == Color(0.4, 0.3, 0.7, 1.0),
		"migration preserves per-leaf custom data")
	TestSupport.check(failed, plant._senescence_batch._count == 1,
