...
func _initialize() -> void:
	...
	TestSupport.check(failed, order.size() == MindModuleGraph.DEPS.size() and not order.is_empty(),
	...
	for node in MindModuleGraph.DEPS.keys():
		for pre in MindModuleGraph.DEPS[node]:
			if not MindModuleGraph.is_before(order, str(pre), str(node)):
				ok_deps = false
	TestSupport.check(failed, ok_deps, "every prerequisite precedes its dependent in the order")

	# Anchored endpoints.
	TestSupport.check(failed, order[0] == "protoself", "protoself ticks first (%s)" % str(order[0]))
	TestSupport.check(failed, order[order.size() - 1] == "encode", "encode ticks last")
	TestSupport.check(failed, MindModuleGraph.is_before(order, "core_affect", "binding")
	...
	TestSupport.check(failed, MindModuleGraph.topo_sort({"a": ["b"], "b": ["a"]}).is_empty(),
			"a cycle yields no order")
	TestSupport.check(failed, not MindModuleGraph.is_valid({"a": ["a"]}), "a self-edge is invalid")
	# A dangling prerequisite is rejected.
	TestSupport.check(failed, MindModuleGraph.topo_sort({"a": ["ghost"]}).is_empty(),
	...
	TestSupport.check(failed, not ext_order.is_empty() and MindModuleGraph.is_before(ext_order, "binding", "mood_weather"),
			"a newly-added module is ordered after its declared dependency")

	quit(TestSupport.report("smoke_module_graph", failed))
