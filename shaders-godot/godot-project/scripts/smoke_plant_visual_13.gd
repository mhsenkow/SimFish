...
func _initialize() -> void:
	...
	TestSupport.check(failed, is_equal_approx(float(mat.get_shader_parameter("golden_hour")), 0.8),
		"full tier receives dawn/dusk rim")
	VoxelMat.update_foliage_golden_hour(0.8, Vector3(1, -1, 0), true)
	TestSupport.check(failed, float(mat.get_shader_parameter("golden_hour")) < 0.35,
		"reduced setting restrains rim")
	VoxelMat.set_shader_perf_tier(2)
	VoxelMat.update_foliage_golden_hour(0.8, Vector3(1, -1, 0), false)
	TestSupport.check(failed, is_zero_approx(float(mat.get_shader_parameter("golden_hour"))),
		"lowest tier disables rim")
	for path in ["res://shaders/foliage.gdshader", "res://shaders/foliage_mm.gdshader"]:
		var source := FileAccess.get_file_as_string(path)
		TestSupport.check(failed, source.contains("sun_edge") and source.contains("foliage_light_dir"),
