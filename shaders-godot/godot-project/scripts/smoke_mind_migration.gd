...
func _initialize() -> void:
	...
	TestSupport.check(failed, int(up.get("schema_version", -1)) == cur, "v2 save migrates to current (%d)" % cur)
	TestSupport.check(failed, up.has("world_model") and up.has("life_stance") and up.has("prediction_error"),
			"migration defaults the v4 extended-channel fields")
	TestSupport.check(failed, is_equal_approx(float(up.get("mood", 0.0)), 0.3) and str(up.get("attention_focus", "")) == "food",
	...
	TestSupport.check(failed, int(same.get("schema_version", -1)) == cur, "current save stays current")
	...
	TestSupport.check(failed, int(fwd.get("schema_version", -1)) == cur + 3, "a newer save is not downgraded")

	# from_dict applies the ladder end-to-end.
	var ms = MindStateScript.new()
	ms.from_dict({"schema_version": 2, "mood": 0.42, "stress": 0.2})
	TestSupport.check(failed, ms.schema_version == cur, "from_dict upgrades schema_version")
	TestSupport.check(failed, is_equal_approx(ms.mood, 0.42), "from_dict keeps migrated values")

	quit(TestSupport.report("smoke_mind_migration", failed))
