...
func _initialize() -> void:
	...
	TestSupport.check(failed, batch != null and batch._mm.use_custom_data,
	...
	TestSupport.check(failed, min_t >= 0.0 and max_t <= 1.0 and max_t > min_t,
	...
	TestSupport.check(failed, shader_source.contains("INSTANCE_CUSTOM.r"),
		"shader reads thickness from custom R")
	TestSupport.check(failed, shader_source.contains("thin_transmission"),
