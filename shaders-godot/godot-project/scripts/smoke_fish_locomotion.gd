...
func _initialize() -> void:
	...
	TestSupport.check(failed, load("res://scripts/fish.gd") != null, "fish.gd must compile")
	TestSupport.check(failed, load("res://scripts/fish_locomotion.gd") != null,
	...
	TestSupport.check(failed, a._wall_avoid(box) == FishLocomotion.wall_avoid(a, box),
	...
	TestSupport.check(failed, a._hardscape_clearance_push() == FishLocomotion.hardscape_clearance_push(a),
	...
	TestSupport.check(failed, hs == Vector3.ZERO and _finite(hs), "hardscape push is ZERO/finite with no sim")

	# --- wall_avoid with no world → ZERO and finite (safe defaults).
	var wa: Vector3 = FishLocomotion.wall_avoid(a, box)
	TestSupport.check(failed, wa == Vector3.ZERO and _finite(wa), "wall_avoid is ZERO/finite with no world")
	...
	TestSupport.check(failed, _finite_f(end_min) and end_min > start_min,
