...
func _initialize() -> void:
	...
	TestSupport.check(failed, is_equal_approx(wave.w, 8.0), "gust radius is capped")
	TestSupport.check(failed, is_equal_approx(float(mat.get_shader_parameter("gust_age")), 3.0),
		"gust age is capped")
	TestSupport.check(failed, is_equal_approx(float(mat.get_shader_parameter("gust_strength")), 1.0),
		"gust strength is capped")
	var source := FileAccess.get_file_as_string("res://shaders/foliage_mm.gdshader")
	TestSupport.check(failed, source.contains("gust_front") and source.contains("gust_fade"),
		"shader contains bounded propagating front")
	var world_source := FileAccess.get_file_as_string("res://scripts/world.gd")
	TestSupport.check(failed, world_source.contains("reduced_motion_enabled()"),
