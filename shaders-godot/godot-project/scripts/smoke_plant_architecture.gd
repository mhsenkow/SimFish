...
func _initialize() -> void:
	...
	TestSupport.check(failed, spiral.phyllotaxis == Plant.PHYLLO_SPIRAL,
	...
	TestSupport.check(failed, absf(d0 - d1) < 0.001, "spiral divergence is constant")
	TestSupport.check(failed, absf(absf(d0) - Plant.GOLDEN_ANGLE) < 0.01
	...
	TestSupport.check(failed, buckets.size() >= 10,
	...
	TestSupport.check(failed, whorled.phyllotaxis == Plant.PHYLLO_WHORLED,
	...
	TestSupport.check(failed, forced.phyllotaxis == Plant.PHYLLO_DECUSSATE,
		"explicit phyllotaxis overrides the derived default")
	# Decussate: opposite pairs, each pair a quarter turn on from the last.
	TestSupport.check(failed, absf(absf(_wrap(forced._phyllotaxis_yaw(1)
			- forced._phyllotaxis_yaw(0))) - PI) < 0.01, "decussate pairs are opposite")
	TestSupport.check(failed, absf(absf(_wrap(forced._phyllotaxis_yaw(2)
	...
	for i in 40:
		...
		p.free()
	TestSupport.check(failed, phases.size() >= 30,
	...
	TestSupport.check(failed, lean_a.distance_to(lean_b) > 0.05,
		"two plants lean in different directions")
	TestSupport.check(failed, absf(lean_a.length() - 1.0) < 0.001, "lean direction is a unit vector")
	...
	TestSupport.check(failed, shade._shade_size_mult() > sun._shade_size_mult(),
		"shade leaves grow larger than sun leaves")
	TestSupport.check(failed, shade._shade_pitch() < sun._shade_pitch(),
		"shade leaves are held flatter")
	TestSupport.check(failed, is_equal_approx(sun._shade_size_mult(), 1.0),
		"full light leaves the leaf at its genome size")
	TestSupport.check(failed, shade._shade_size_mult() < 1.4, "shade response stays bounded")
	...
	TestSupport.check(failed, mid_sz > base_sz and mid_sz > tip_sz,
		"largest leaves sit mid-stem (%.2f base / %.2f mid / %.2f tip)"
			% [base_sz, mid_sz, tip_sz])
	TestSupport.check(failed, base_sz > 0.5 and tip_sz > 0.5, "gradient never collapses a leaf")

	# ---- Node jitter is deterministic and small ------------------------------
	TestSupport.check(failed, is_equal_approx(spiral._node_jitter(5, 0.1), spiral._node_jitter(5, 0.1)),
		"node jitter is stable for a node")
	TestSupport.check(failed, not is_equal_approx(spiral._node_jitter(5, 0.1), spiral._node_jitter(6, 0.1)),
		"node jitter differs between nodes")
	TestSupport.check(failed, absf(spiral._node_jitter(5, 0.1)) <= 0.1001, "node jitter respects its bound")

	# ---- Leaf mosaic: twist toward the lamp ----------------------------------
	# With no light resolved (headless) the leaf keeps its divergence angle.
	TestSupport.check(failed, is_equal_approx(spiral._leaf_yaw(3, 0.0),
	...
	TestSupport.check(failed, not is_equal_approx(y3, spiral._phyllotaxis_yaw(3)),
		"a lamp twists the leaf off its divergence angle")
	TestSupport.check(failed, absf(_wrap(y3 - y4)) > 0.5,
		"the twist does not collapse the arrangement into one plane")
	# The twist must be partial, never a full snap to the lamp.
	TestSupport.check(failed, absf(_wrap(y3 - 1.0)) > 0.2,
	...
	TestSupport.check(failed, seg0 > 0, "epiphyte builds a rhizome at spawn")
	var attach0: int = epi._rhizome_attach_points.size()
	for i in 12:
		epi._grow_one()
	TestSupport.check(failed, epi._rhizome_segments > seg0,
		"growth extends the rhizome along the host (%d -> %d)" % [seg0, epi._rhizome_segments])
	TestSupport.check(failed, epi._rhizome_attach_points.size() > attach0,
	...
	if pts.size() >= 3:
		...
		TestSupport.check(failed, a.dot(b) < 0.9999, "the rhizome curves as it creeps")
	...
	for i in 200:
		...
		e2.free()
	TestSupport.check(failed, straightest >= Plant.RHIZOME_CURL_MIN - 0.0001,
		"every individual curls at least the minimum (worst %.4f)" % straightest)
	# And it must stop somewhere.
	for i in 200:
		epi._extend_rhizome()
	TestSupport.check(failed, epi._rhizome_segments <= Plant.RHIZOME_MAX_SEGMENTS,
	...
	TestSupport.check(failed, PlantGenome.DEFAULTS_PHYLLO_SPIRAL == Plant.PHYLLO_SPIRAL
	...
	TestSupport.check(failed, ribbon_tip > sword_tip,
	...
	TestSupport.check(failed, carpet_flutter > 0.02 and carpet_speed >= 3.5,
	...
	TestSupport.check(failed, not bud_nodes.is_empty(), "bud builds voxels")
	var bud_mat: ShaderMaterial = (bud_nodes[0] as MeshInstance3D).material_override as ShaderMaterial
	TestSupport.check(failed, bud_mat != null, "bud has a shader material")
	var bud_amp: float = float(bud_mat.get_shader_parameter("sway_amplitude"))
	TestSupport.check(failed, bud_amp <= 0.0001,
		"bud flower mat has zero sway (got %.3f)" % bud_amp)
	TestSupport.check(failed, float(bud_mat.get_shader_parameter("motion_lock")) > 0.5,
	...
	TestSupport.check(failed, tip_plant._flower_node != null and is_instance_valid(tip_plant._flower_node),
	...
	TestSupport.check(failed, tip_plant._flower_node.position.distance_to(expected) < 0.001,
		"flower snaps to tip bloom local pos")
	TestSupport.check(failed, tip_plant._flower_node.position.length() > 0.01
