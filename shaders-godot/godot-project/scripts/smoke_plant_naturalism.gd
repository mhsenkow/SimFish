...
func _initialize() -> void:
	...
	TestSupport.check(failed, int(seed_kid.generation) == 0 or true, "mutate returns enrich")
	...
	seed_drift /= 24.0
	clone_drift /= 24.0
	TestSupport.check(failed, clone_drift < seed_drift,
		"clone drift (%.3f) < seed drift (%.3f)" % [clone_drift, seed_drift])
	TestSupport.check(failed, PlantGenome.drift_distance(base, base) < 0.02,
	...
	TestSupport.check(failed, lid != "", "lineage id assigned")
	TestSupport.check(failed, int(reg.get_entry(lid).get("count", 0)) == 1, "count=1 after register")
	reg.register_genome(clone_kid)
	TestSupport.check(failed, reg.snapshot().size() >= 1, "snapshot non-empty")
	...
	TestSupport.check(failed, p._tropism.length() > 0.5, "tropism is unit-ish after init")
	p._refresh_tropism()
	var off: Vector2 = p._tropism_lateral_offset()
	TestSupport.check(failed, off.length() < 0.5, "tropism lateral offset is a small lean")
	# Grow enough leaves to register lifecycle state.
	for _g in 6:
		p._grow_one()
	TestSupport.check(failed, p._leaf_states.size() > 0, "leaves registered with lifecycle state")
	if not p._leaf_states.is_empty():
		var st: Dictionary = p._leaf_states[0]
		TestSupport.check(failed, st.has("phase"), "leaf has phase field")
		# Fast-forward through expand → mature.
		st.age_s = Plant.LEAF_EXPAND_S + 1.0
		p._advance_leaf_phase(st, 0)
		TestSupport.check(failed, int(st.phase) == Plant.LeafPhase.MATURE,
			"EXPANDING→MATURE after expand window")
		st.age_s = Plant.LEAF_EXPAND_S + Plant.LEAF_MATURE_S + 1.0
		p._advance_leaf_phase(st, 0)
		TestSupport.check(failed, int(st.phase) == Plant.LeafPhase.SENESCENT,
	...
	TestSupport.check(failed, is_equal_approx(c0.r, c0b.r) and is_equal_approx(c0.g, c0b.g),
		"micro-vary is deterministic for a key")
	TestSupport.check(failed, absf(c0.h - c1.h) > 1e-6 or absf(c0.v - c1.v) > 1e-6,
