...
func _initialize() -> void:
	...
	TestSupport.check(failed, is_equal_approx(float(a._world_model["gru_error"]), float(b._world_model["gru_error"])),
			"same-id GRU is deterministic across 100 ticks")

	var hsum: float = 0.0
	for x in (a._world_model["h"] as PackedFloat32Array):
		hsum += absf(x)
	TestSupport.check(failed, hsum > 0.01, "GRU hidden state evolves (recurrence is live, not stuck at 0)")
	...
	for _i in 10:
		for fsh in [a, c]:
			...
			fsh.curiosity_drive = 0.6
			MindWorldModel.tick(fsh, null, 0.1)
	TestSupport.check(failed, is_equal_approx(float(a._world_model["gru_error"]), float(c._world_model["gru_error"])),
	...
	TestSupport.check(failed, d._world_model.has("h") and d._world_model.has("gw"),
			"legacy save backfills GRU fields (migration)")
	MindWorldModel.tick(d, null, 0.1)   # must not crash on a migrated model
	TestSupport.check(failed, is_finite(float(d._world_model["gru_error"])), "migrated model ticks cleanly")

	quit(TestSupport.report("smoke_world_model_gru", failed))
