...
func _initialize() -> void:
	...
	TestSupport.check(failed, image.get_width() == 16 and image.get_height() == 8,
		"canopy input stays low resolution")
	var occupied: bool = false
	for y in image.get_height():
		for x in image.get_width():
			occupied = occupied or image.get_pixel(x, y).r > 0.0
	TestSupport.check(failed, occupied, "living crown contributes density")
	...
	TestSupport.check(failed, builder.contains("PLANT_CAP: int = 128"),
		"canopy scan is capped")
	TestSupport.check(failed, source.contains("_canopy_density_next_ms = now_ms + 3000"),
		"input updates coarsely")
	TestSupport.check(failed, source.contains("shader_perf_tier() < 2"),
