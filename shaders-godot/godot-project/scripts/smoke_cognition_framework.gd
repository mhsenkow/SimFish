...
func _initialize() -> void:
	...
	TestSupport.check(failed, MindAblation.enabled(MindAblation.WORLD_MODEL), "modules enabled by default")
	MindAblation.set_enabled(MindAblation.WORLD_MODEL, false)
	TestSupport.check(failed, not MindAblation.enabled(MindAblation.WORLD_MODEL), "set_enabled(false) disables")
	MindAblation.reset()
	TestSupport.check(failed, MindAblation.enabled(MindAblation.WORLD_MODEL), "reset re-enables")
	...
	TestSupport.check(failed, MindTrace.size() == 0, "disabled trace records nothing (zero cost)")
	...
	TestSupport.check(failed, MindTrace.size() == MindTrace.CAP, "ring buffer is cap-bounded")
	var tail: Array = MindTrace.recent(3)
	TestSupport.check(failed, tail.size() == 3 and int(tail[2]["n"]) == MindTrace.CAP + 24,
	...
	TestSupport.check(failed, MindTrace.size() >= 1, "a cognitive cycle emits a trace event")
	if MindTrace.size() >= 1:
		var ev: Dictionary = MindTrace.recent(1)[0]
		TestSupport.check(failed, ev.has("focus") and ev.has("ignited") and str(ev.get("id", "")) == "traced",
				"trace event carries focus/ignition and the fish id")

	MindTrace.set_enabled(false)
	MindAblation.reset()

	quit(TestSupport.report("smoke_cognition_framework", failed))
