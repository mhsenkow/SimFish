...
func _initialize() -> void:
	...
	TestSupport.check(failed, MindBoidsBuffer.backend == "cpu", "topo schooling uses cpu backend")

	var betta_idx: int = MindBoidsBuffer.index_for(betta)
	TestSupport.check(failed, betta_idx >= 0, "betta captured in boids buffer")
	TestSupport.check(failed, not MindBoidsBuffer.uses_topo_at(betta_idx), "betta excluded from topo path")
	TestSupport.check(failed, MindBoidsBuffer.neighbor_counts[betta_idx] == 0,
	...
	for t in tetras:
		...
		TestSupport.check(failed, MindBoidsBuffer.uses_topo_at(idx), "tetra on topo path")
		var nc: int = MindBoidsBuffer.neighbor_counts[idx]
		TestSupport.check(failed, nc > 0 and nc <= MindBoidsBuffer.N_TOPO,
		...
		for s in MindBoidsBuffer.N_TOPO:
			if MindBoidsBuffer.topo_neighbor_at(idx, s) >= 0:
				topo_slots += 1
	TestSupport.check(failed, topo_slots >= 6, "topological links materialized (%d slots)" % topo_slots)
	...
	for t in tetras:
		if t.motion_agitation > 0.2:
			tetra_hot += 1
	TestSupport.check(failed, tetra_hot >= 1, "startle injects agitation into schoolers")
	TestSupport.check(failed, betta.motion_agitation < 0.05,
	...
	for t in tetras:
		if t.motion_agitation > 0.08:
			spread += 1
	TestSupport.check(failed, spread >= 2, "agitation propagates through topo links (%d fish)" % spread)
	...
	for t in tetras:
		if t.motion_agitation > 0.08:
			spread_ff += 1
	TestSupport.check(failed, spread_ff >= 2,
	...
	TestSupport.check(failed, not _MotionWave.uses_wave(angel), "hover angelfish skips wave path")
	all.append(angel)
	MindBoidsBuffer.capture(all, 2)
	MindBoidsCompute.run()
	var angel_idx: int = MindBoidsBuffer.index_for(angel)
	TestSupport.check(failed, angel_idx >= 0, "hover fish captured")
	TestSupport.check(failed, not MindBoidsBuffer.uses_topo_at(angel_idx), "hover fish off topo path")
	TestSupport.check(failed, MindBoidsBuffer.neighbor_counts[angel_idx] == 0,
	...
	TestSupport.check(failed, tetras[0]._boids_neighbor_count == 0,
		"non-conspecifics do not ali/coh (count=%d)" % tetras[0]._boids_neighbor_count)
	TestSupport.check(failed, steer.length_squared() > 1e-5,
	...
	parent.queue_free()
	quit(TestSupport.report("smoke_murmuration", failed))
