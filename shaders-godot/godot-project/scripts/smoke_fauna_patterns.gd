...
func _initialize() -> void:
	...
	for ptype in [11, 12, 13, 14]:
		_assert(failed, _voxel_count(host, ptype, "new%d" % ptype) > solid_count,
	...
	_assert(failed, a1 == a2, "speckle is deterministic for one id")
	var b1: int = _voxel_count(host, 14, "star_b_long_distinct_id")
	_assert(failed, b1 > 0, "second speckled fish builds")
	...
	for i in 64:
		...
		_assert(failed, h >= 0, "hash is non-negative for %s" % f.id)
		seen[h] = true
		f.free()
	_assert(failed, seen.size() >= 60, "pattern hash spreads across ids (%d/64)" % seen.size())
	...
	_assert(failed, a1d["pec_r"] == a1_again["pec_r"], "asymmetry is stable for one fish")
	_assert(failed, not is_equal_approx(float(a1d["pec_r"]), float(a1d["pec_l"])),
		"the two pectorals differ")
	_assert(failed, is_equal_approx(float(a1d["pec_r"]) + float(a1d["pec_l"]), 2.0),
		"pectoral deviation is balanced around 1.0")
	_assert(failed, absf(float(a1d["pec_r"]) - 1.0) < 0.10,
	...
	for i in 200:
		...
		if bool(a["nick"]):
			nicks += 1
		_assert(failed, absf(float(a["eye_lift"])) <= 0.031, "eye lift stays subtle")
		fx.free()
	_assert(failed, leans.size() >= 20,
		"pectoral lean varies across the population (%d distinct)" % leans.size())
	_assert(failed, nicks > 10 and nicks < 80,
	...
	_assert(failed, wild == solid_count, "unknown pattern id degrades to solid")
