...
func _initialize() -> void:
	...
	for path in [
		...
		_assert(failed, source.contains("aquatic_caustic_intensity"),
			"%s exposes caustic intensity" % path)
		_assert(failed, source.contains("cwave = sin"),
			"%s contains bounded lightweight wave math" % path)
		_assert(failed, not source.contains("for (int"),
	...
	_assert(failed, is_equal_approx(float(
