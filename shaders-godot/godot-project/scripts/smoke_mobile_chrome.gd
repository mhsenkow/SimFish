...
func _initialize() -> void:
	...
	_assert(failed, SafeArea.insets(null) == SafeArea.ZERO, "null viewport -> zero insets")

	SafeArea.set_test_override(Vector4(0.0, 47.0, 0.0, 34.0))
	var pad: Vector4 = SafeArea.insets(root)
	_assert(failed, is_equal_approx(pad.y, 47.0), "override reports top inset")
	_assert(failed, is_equal_approx(pad.w, 34.0), "override reports bottom inset")
	...
	_assert(failed, neg.x >= 0.0 and neg.y >= 0.0 and neg.z >= 0.0 and neg.w >= 0.0,
	...
	_assert(failed, is_equal_approx(r.position.x, 10.0) and is_equal_approx(r.position.y, 20.0),
		"safe rect origin follows insets")
	_assert(failed, is_equal_approx(r.size.x, view.x - 40.0), "safe rect width excludes L+R")
	_assert(failed, is_equal_approx(r.size.y, view.y - 60.0), "safe rect height excludes T+B")

	# The cap keeps one cutout from eating the whole screen even if the
	# platform hands back garbage.
	_assert(failed, SafeArea.MAX_INSET_FRACTION > 0.0 and SafeArea.MAX_INSET_FRACTION <= 0.25,
	...
	_assert(failed, src.contains("func _safe_pad()"), "main exposes _safe_pad()")
	_assert(failed, src.contains("func _apply_footer_layout()"),
		"footer geometry is re-appliable on resize")
	_assert(failed, src.contains("_apply_footer_layout()\n\t_apply_panel_layout()"),
	...
	for marker in ["stats_bar.offset_top = 4.0 + pad.y",
		...
		_assert(failed, src.contains(marker), "safe-area applied: %s" % marker)
	...
	_assert(failed, menu.contains("SafeArea.insets(get_viewport())"),
		"tank menu reads the safe area")
	_assert(failed, menu.contains("- pad.x - pad.z"),
	...
	_assert(failed, not mh.contains("DisplayServer.get_display_safe_area()"),
		"mobile_hud delegates safe area to SafeArea")
	_assert(failed, mh.contains("SafeArea.rect("), "mobile_hud reads shared safe rect")

	# ---- Physical touch targets ---------------------------------------------
	_assert(failed, PanelTheme.MIN_TOUCH_INCHES >= 0.25,
	...
	if not PanelTheme.is_touch_device():
		_assert(failed, is_equal_approx(PanelTheme.min_touch_px(root), 0.0),
			"desktop has no touch floor")
		_assert(failed, is_equal_approx(PanelTheme.touch_size(root, 48.0), 48.0),
			"desktop button size unchanged")
	_assert(failed, PanelTheme.touch_size(root, 48.0) >= 48.0,
		"touch sizing never shrinks a control")
	# The cap has to bound growth, otherwise a mis-reported DPI blows up the HUD.
	_assert(failed, PanelTheme.touch_size(root, 48.0) <= 48.0 * 2.4 + 0.001,
	...
	_assert(failed, inset >= PanelTheme.rail_button_size(root),
		"rail chrome inset clears the rail button")
	_assert(failed, is_equal_approx(PanelTheme.rail_chrome_inset(),
			PanelTheme.RAIL_WIDTH + PanelTheme.EDGE_MARGIN + PanelTheme.RAIL_CLEARANCE),
		"no-arg rail inset keeps the desktop constant")

	SafeArea.clear_test_override()
	_assert(failed, SafeArea.insets(root) == SafeArea.ZERO or PanelTheme.is_touch_device(),
