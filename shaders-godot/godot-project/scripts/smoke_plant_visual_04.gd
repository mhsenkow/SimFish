...
func _initialize() -> void:
	...
	for path in ["res://shaders/foliage.gdshader", "res://shaders/foliage_mm.gdshader"]:
		...
		TestSupport.check(failed, source.contains("wet_band") and source.contains("wet_view"),
	...
	TestSupport.check(failed, float(mat.get_shader_parameter("wet_sheen_strength")) <= 0.10,
		"lowest tier reduces wet sheen")
	VoxelMat.set_shader_perf_tier(0)
	TestSupport.check(failed, float(mat.get_shader_parameter("wet_sheen_strength")) >= 0.30,
