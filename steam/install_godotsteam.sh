...
VERSION="${GODOTSTEAM_VERSION:-v4.19-gde}"
ZIP="godotsteam-4.19-gdextension-plugin-4.4.zip"
URL="https://codeberg.org/GodotSteam/GodotSteam/releases/download/${VERSION}/${ZIP}"

if [[ -f "$ADDON/godotsteam.gdextension" ]]; then
	echo "GodotSteam already installed at $ADDON"
	exit 0
fi
